        ...
        for retry in range(tryCount):
            try:
                ...
                self.IUIAutomation = comtypes.client.CreateObject(
                    "{ff48dba4-60ef-4201-aa87-54103eef594e}", interface=self.UIAutomationCore.IUIAutomation)
    ...
    def __init__(self):
        ...
        if load:
            ...
            self.dll.BitmapToFile.argtypes = (
                ctypes.c_size_t, ctypes.c_wchar_p, ctypes.c_wchar_p)
            ...
            self.dll.BitmapSetPixel.argtypes = (
                ctypes.c_size_t, ctypes.c_int, ctypes.c_int, ctypes.c_uint)
...
def SetClipboardText(text: str) -> bool:
    ...
    if ctypes.windll.user32.OpenClipboard(0):
        ...
        hClipboardData = ctypes.windll.kernel32.GlobalAlloc(
            0, textByteLen)  # GMEM_FIXED=0
        hDestText = ctypes.windll.kernel32.GlobalLock(
            ctypes.c_void_p(hClipboardData))
        ctypes.cdll.msvcrt.wcsncpy(ctypes.c_wchar_p(hDestText), ctypes.c_wchar_p(
            text), ctypes.c_size_t(textByteLen // 2))
        ...
        ctypes.windll.user32.SetClipboardData(ctypes.c_uint(
            13), ctypes.c_void_p(hClipboardData))  # CF_TEXT=1, CF_UNICODETEXT=13
        ctypes.windll.user32.CloseClipboard()
        return True
    return False
...
def Click(x: int, y: int, waitTime: float = OPERATION_WAIT_TIME) -> None:
    ...
    mouse_event(MouseEventFlag.LeftDown | MouseEventFlag.Absolute,
                x * 65535 // screenWidth, y * 65535 // screenHeight, 0, 0)
    time.sleep(0.05)
...
def MiddleClick(x: int, y: int, waitTime: float = OPERATION_WAIT_TIME) -> None:
    ...
    SetCursorPos(x, y)
    screenWidth, screenHeight = GetScreenSize()
    mouse_event(MouseEventFlag.MiddleDown | MouseEventFlag.Absolute,
                x * 65535 // screenWidth, y * 65535 // screenHeight, 0, 0)
    time.sleep(0.05)
    mouse_event(MouseEventFlag.MiddleUp | MouseEventFlag.Absolute,
                x * 65535 // screenWidth, y * 65535 // screenHeight, 0, 0)
    time.sleep(waitTime)
...
def GetMonitorsRect(dpiAwarenessPerMonitor: bool = False) -> List[Rect]:
    ...
    MonitorEnumProc = ctypes.WINFUNCTYPE(
        ctypes.c_int, ctypes.c_size_t, ctypes.c_size_t, ctypes.POINTER(ctypes.wintypes.RECT), ctypes.c_size_t)
    rects = []

    def MonitorCallback(hMonitor: int, hdcMonitor: int, lprcMonitor: ctypes.POINTER(ctypes.wintypes.RECT), dwData: int):
        rect = Rect(lprcMonitor.contents.left, lprcMonitor.contents.top,
                    lprcMonitor.contents.right, lprcMonitor.contents.bottom)
        rects.append(rect)
        return 1
    ret = ctypes.windll.user32.EnumDisplayMonitors(ctypes.c_void_p(
        0), ctypes.c_void_p(0), MonitorEnumProc(MonitorCallback), 0)
    return rects
...
def GetPixelColor(x: int, y: int, handle: int = 0) -> int:
    ...
    ctypes.windll.user32.ReleaseDC(
        ctypes.c_void_p(handle), ctypes.c_void_p(hdc))
    return bgr
...
def SwitchToThisWindow(handle: int) -> None:
    ...
    handle: int, the handle of a native window.
    """
    ctypes.windll.user32.SwitchToThisWindow(ctypes.c_void_p(
        handle), ctypes.c_int(1))  # void function, no return
...
def GetConsoleOriginalTitle() -> str:
    ...
    if IsNT6orHigher:
        arrayType = ctypes.c_wchar * MAX_PATH
        values = arrayType()
        ctypes.windll.kernel32.GetConsoleOriginalTitleW(
            values, ctypes.c_uint(MAX_PATH))
        return values.value
    else:
        raise RuntimeError(
...
def IsDesktopLocked() -> bool:
    ...
    """
    isLocked = False
    desk = ctypes.windll.user32.OpenDesktopW(ctypes.c_wchar_p('Default'), ctypes.c_uint(
        0), ctypes.c_int(0), ctypes.c_uint(0x0100))  # DESKTOP_SWITCHDESKTOP = 0x0100
    if desk:
        isLocked = not ctypes.windll.user32.SwitchDesktop(
            ctypes.c_void_p(desk))
...
def IsProcess64Bit(processId: int) -> bool:
    ...
    try:
        ...
        func = ctypes.windll.ntdll.ZwWow64ReadVirtualMemory64
    except Exception as ex:
        return False
    try:
...
def SendKey(key: int, waitTime: float = OPERATION_WAIT_TIME) -> None:
    ...
    """
    keybd_event(key, 0, KeyboardEventFlag.KeyDown |
                KeyboardEventFlag.ExtendedKey, 0)
    keybd_event(key, 0, KeyboardEventFlag.KeyUp |
                KeyboardEventFlag.ExtendedKey, 0)
    time.sleep(waitTime)
...
def _VKtoSC(key: int) -> int:
    ...
    if not scanCode:
        return 0
    keyList = [Keys.VK_APPS, Keys.VK_CANCEL,
               Keys.VK_SNAPSHOT, Keys.VK_DIVIDE, Keys.VK_NUMLOCK]
    if key in keyList:
        scanCode |= 0x0100
    return scanCode
...
def SendKeys(text: str, interval: float = 0.01, waitTime: float = OPERATION_WAIT_TIME, charMode: bool = True, debug: bool = False) -> None:
    ...
    SendKeys('[]{{}{}}\\|;:\'\",<.>/?{Enter}')
    """
    holdKeys = ('WIN', 'LWIN', 'RWIN', 'SHIFT', 'LSHIFT', 'RSHIFT', 'CTRL',
                'CONTROL', 'LCTRL', 'RCTRL', 'LCONTROL', 'LCONTROL', 'ALT', 'LALT', 'RALT')
    keys = []
    printKeys = []
    i = 0
    insertIndex = 0
    ...
    while True:
        if text[i] == '{':
            ...
            if (len(key) == 2 and not key[1].isdigit()) or len(key) > 2:
                raise ValueError(
                    '"{}" is not valid'.format(text[i:rindex + 1]))
                ...
                if hold:
                    if upperKey in SpecialKeyNames:
                        ...
                        if type(lastKeyValue) == type(keyValue) and lastKeyValue == keyValue:
                            insertIndex += 1
                        printKeys.insert(
                            insertIndex, (key[0], 'KeyDown | ExtendedKey'))
                        printKeys.insert(
                            insertIndex + 1, (key[0], 'KeyUp | ExtendedKey'))
                        keys.insert(
                            insertIndex, (keyValue, KeyboardEventFlag.KeyDown | KeyboardEventFlag.ExtendedKey))
                        keys.insert(
                            insertIndex + 1, (keyValue, KeyboardEventFlag.KeyUp | KeyboardEventFlag.ExtendedKey))
            ...
            if hold:
                if text[i] in CharacterCodes:
                    ...
                    if include and type(lastKeyValue) == type(keyValue) and lastKeyValue == keyValue:
                        insertIndex += 1
                    printKeys.insert(
                        insertIndex, (text[i], 'KeyDown | ExtendedKey'))
                    printKeys.insert(
                        insertIndex + 1, (text[i], 'KeyUp | ExtendedKey'))
                    keys.insert(
                        insertIndex, (keyValue, KeyboardEventFlag.KeyDown | KeyboardEventFlag.ExtendedKey))
                    keys.insert(
                        insertIndex + 1, (keyValue, KeyboardEventFlag.KeyUp | KeyboardEventFlag.ExtendedKey))
    ...
    for i, key in enumerate(keys):
        if key[1] == 'UnicodeChar':
            ...
            if debug:
                Logger.Write(
                    printKeys[i], ConsoleColor.DarkGreen, writeToFile=False)
            if i + 1 == len(keys):
                time.sleep(interval)
                if debug:
                    Logger.Write(', sleep({})\n'.format(
                        interval), writeToFile=False)
            else:
                if key[1] & KeyboardEventFlag.KeyUp:
                    if keys[i + 1][1] == 'UnicodeChar' or keys[i + 1][1] & KeyboardEventFlag.KeyUp == 0:
                        ...
                        if debug:
                            Logger.Write(', sleep({})\n'.format(
                                hotkeyInterval), writeToFile=False)
...
class Bitmap:
    ...
    def FromHandle(self, hwnd: int, left: int = 0, top: int = 0, right: int = 0, bottom: int = 0) -> bool:
        ...
        left, top, right, bottom = left + rect.left, top + \
            rect.top, right + rect.left, bottom + rect.top
    ...
    def ToFile(self, savePath: str) -> bool:
        ...
        extMap = {'.bmp': 'image/bmp', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.gif': 'image/gif', '.tif': 'image/tiff', '.tiff': 'image/tiff', '.png': 'image/png'
                  }
...
def GetPatternIdInterface(patternId: int):
    ...
    if not _PatternIdInterfaces:
        ...
        try:
            _PatternIdInterfaces[PatternId.AnnotationPattern] = _AutomationClient.instance(
            ).UIAutomationCore.IUIAutomationAnnotationPattern
        except:
            if debug:
                Logger.WriteLine(
                    'UIAutomationCore does not have AnnotationPattern.', ConsoleColor.Yellow)
        try:
            _PatternIdInterfaces[PatternId.CustomNavigationPattern] = _AutomationClient.instance(
            ).UIAutomationCore.IUIAutomationCustomNavigationPattern
        except:
            if debug:
                Logger.WriteLine(
                    'UIAutomationCore does not have CustomNavigationPattern.', ConsoleColor.Yellow)
        try:
            _PatternIdInterfaces[PatternId.DragPattern] = _AutomationClient.instance(
            ).UIAutomationCore.IUIAutomationDragPattern
        except:
            if debug:
                Logger.WriteLine(
                    'UIAutomationCore does not have DragPattern.', ConsoleColor.Yellow)
        try:
            _PatternIdInterfaces[PatternId.DropTargetPattern] = _AutomationClient.instance(
            ).UIAutomationCore.IUIAutomationDropTargetPattern
        except:
            if debug:
                Logger.WriteLine(
                    'UIAutomationCore does not have DropTargetPattern.', ConsoleColor.Yellow)
        try:
            _PatternIdInterfaces[PatternId.ObjectModelPattern] = _AutomationClient.instance(
            ).UIAutomationCore.IUIAutomationObjectModelPattern
        except:
            if debug:
                Logger.WriteLine(
                    'UIAutomationCore does not have ObjectModelPattern.', ConsoleColor.Yellow)
        try:
            _PatternIdInterfaces[PatternId.SpreadsheetItemPattern] = _AutomationClient.instance(
            ).UIAutomationCore.IUIAutomationSpreadsheetItemPattern
        except:
            if debug:
                Logger.WriteLine(
                    'UIAutomationCore does not have SpreadsheetItemPattern.', ConsoleColor.Yellow)
        try:
            _PatternIdInterfaces[PatternId.SpreadsheetPattern] = _AutomationClient.instance(
            ).UIAutomationCore.IUIAutomationSpreadsheetPattern
        except:
            if debug:
                Logger.WriteLine(
                    'UIAutomationCore does not have SpreadsheetPattern.', ConsoleColor.Yellow)
        try:
            _PatternIdInterfaces[PatternId.StylesPattern] = _AutomationClient.instance(
            ).UIAutomationCore.IUIAutomationStylesPattern
        except:
            if debug:
                Logger.WriteLine(
                    'UIAutomationCore does not have StylesPattern.', ConsoleColor.Yellow)
        try:
            _PatternIdInterfaces[PatternId.TextChildPattern] = _AutomationClient.instance(
            ).UIAutomationCore.IUIAutomationTextChildPattern
        except:
            if debug:
                Logger.WriteLine(
                    'UIAutomationCore does not have TextChildPattern.', ConsoleColor.Yellow)
        try:
            _PatternIdInterfaces[PatternId.TextEditPattern] = _AutomationClient.instance(
            ).UIAutomationCore.IUIAutomationTextEditPattern
        except:
            if debug:
                Logger.WriteLine(
                    'UIAutomationCore does not have TextEditPattern.', ConsoleColor.Yellow)
        try:
            _PatternIdInterfaces[PatternId.TextPattern2] = _AutomationClient.instance(
            ).UIAutomationCore.IUIAutomationTextPattern2
        except:
            if debug:
                Logger.WriteLine(
                    'UIAutomationCore does not have TextPattern2.', ConsoleColor.Yellow)
        try:
            _PatternIdInterfaces[PatternId.TransformPattern2] = _AutomationClient.instance(
            ).UIAutomationCore.IUIAutomationTransformPattern2
        except:
            if debug:
                Logger.WriteLine(
                    'UIAutomationCore does not have TransformPattern2.', ConsoleColor.Yellow)
...
class ScrollPattern():
    ...
    def SetScrollPercent(self, horizontalPercent: float, verticalPercent: float, waitTime: float = OPERATION_WAIT_TIME) -> bool:
        ...
        """
        ret = self.pattern.SetScrollPercent(
            horizontalPercent, verticalPercent) == S_OK
        time.sleep(waitTime)
        return ret
...
class TextRange():
    ...
    def FindAttribute(self, textAttributeId: int, val, backward: bool) -> 'TextRange':
        ...
        """
        textRange = self.textRange.FindAttribute(
            textAttributeId, val, int(backward))
        if textRange:
    ...
    def FindText(self, text: str, backward: bool, ignoreCase: bool) -> 'TextRange':
        ...
        """
        textRange = self.textRange.FindText(
            text, int(backward), int(ignoreCase))
        if textRange:
            return TextRange(textRange=textRange)
    ...
    def MoveEndpointByRange(self, srcEndPoint: int, textRange: 'TextRange', targetEndPoint: int, waitTime: float = OPERATION_WAIT_TIME) -> bool:
        ...
        """
        ret = self.textRange.MoveEndpointByRange(
            srcEndPoint, textRange.textRange, targetEndPoint) == S_OK
        time.sleep(waitTime)
        return ret
...
class Control():
    ValidKeys = set(['ControlType', 'ClassName', 'AutomationId',
                     'Name', 'SubName', 'RegexName', 'Depth', 'Compare'])
    ...
    def CreateControlFromElement(element) -> 'Control':
        ...
        if element:
            ...
            if controlType in ControlConstructors:
                ...
                Logger.WriteLine("element.CurrentControlType returns {}, invalid ControlType!".format(
                    controlType), ConsoleColor.Red)  # rarely happens
    ...
    def GetSearchPropertiesStr(self) -> str:
        strs = ['{}: {}'.format(k, ControlTypeNames[v] if k == 'ControlType' else repr(
            v)) for k, v in self.searchProperties.items()]
        return '{' + ', '.join(strs) + '}'
    ...
    def Exists(self, maxSearchSeconds: float = 5, searchIntervalSeconds: float = SEARCH_INTERVAL, printIfNotExist: bool = False) -> bool:
        ...
        while True:
            control = FindControl(
                self.searchFromControl, self._CompareFunction, self.searchDepth, False, self.foundIndex)
            if control:
                self._element = control.Element
...
class AppBarControl(Control):
    def __init__(self, searchFromControl: Control = None, searchDepth: int = 0xFFFFFFFF, searchInterval: float = SEARCH_INTERVAL, foundIndex: int = 1, element=None, **searchProperties):
        Control.__init__(self, searchFromControl, searchDepth,
                         searchInterval, foundIndex, element, **searchProperties)
            ...
            if listControl.Exists(1):
                ...
                if listItemControl.Exists(0, 0):
                    ...
                    listItemControl.Click(
                        simulateMove=simulateMove, waitTime=waitTime)
                    find = True
        if not find:
            Logger.ColorfullyLog(
                'Can\'t find <Color=Cyan>{}</Color> in ComboBoxControl or it does not support selection.'.format(itemName), ConsoleColor.Yellow)
            if expandCollapsePattern:
                expandCollapsePattern.Collapse(waitTime)
            else:
                self.Click(x=-10, ratioY=0.5,
                           simulateMove=simulateMove, waitTime=waitTime)
        return find
...
class TopLevel():
    ...
    def MoveToCenter(self) -> bool:
        ...
        if self.IsTopLevel():
            rect = self.BoundingRectangle
            screenWidth, screenHeight = GetScreenSize()
            x, y = (screenWidth - rect.width()
                    ) // 2, (screenHeight - rect.height()) // 2
            if x < 0:
                x = 0
            if y < 0:
                y = 0
...
class WindowControl(Control, TopLevel):
    ...
    def MetroClose(self, waitTime: float = OPERATION_WAIT_TIME) -> None:
        ...
        if self.ClassName == METRO_WINDOW_CLASS_NAME:
            ...
            DragDrop(screenWidth // 2, 0, screenWidth //
                     2, screenHeight, waitTime=waitTime)
        else:
...
def LogControl(control: Control, depth: int = 0, showAllName: bool = True, showPid: bool = False) -> None:
    ...
    supportedPatterns = list(filter(lambda t: t[0], ((
                ...
                if msg.lParam & 0x0000FFFF == id2HotKey[msg.wParam][0] and msg.lParam >> 16 & 0x0000FFFF == id2HotKey[msg.wParam][1]:
                    ...
                    if not id2Thread[msg.wParam]:
                        ...
                        if id2Thread[msg.wParam].is_alive():
                            Logger.WriteLine('There is a {} that is already running for hotkey {}'.format(
                                id2Thread[msg.wParam], id2Name[msg.wParam]), ConsoleColor.Yellow, writeToFile=False)
                        else:
                            stopEvent.clear()
                            funcThread = threading.Thread(None, threadFunc, args=(
                                id2Function[msg.wParam], stopEvent, id2HotKey[msg.wParam], id2Name[msg.wParam]))
                    ...
                    for id_ in id2Thread:
                        if id2Thread[id_]:
                            if id2Thread[id_].is_alive():
                                livingThreads.append(
                                    (id2Thread[id_], id2Name[id_]))
                            id2Thread[id_] = None
    ...
    for thread, hotkeyName in livingThreads:
        if thread.is_alive():
            Logger.Write('join {} triggered by hotkey {}\n'.format(
                thread, hotkeyName), ConsoleColor.DarkYellow, writeToFile=False)
            thread.join(2)
    os._exit(0)
...
if __name__ == '__main__':
    ...
    for k, v in PropertyIdNames.items():
        try:
            ...
            print('GetPropertyValue, {} = {}, type: {}'.format(
                v, value, type(value)))
